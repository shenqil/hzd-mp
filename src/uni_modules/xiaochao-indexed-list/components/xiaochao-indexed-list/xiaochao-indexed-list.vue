<template>
	<view>
		<!-- 右边滑动字母 -->
		<view class="a-b" hover-class="a-b-hover" v-bind:style="[{height:zimugaodu+'px'}]" hover-start-time="1" hover-stay-time="100" @touchstart.stop.prevent="touchstarta" @touchmove.stop.prevent="touchmovea" @touchend.stop.prevent="touchenda">
		...
		<!-- #ifdef VUE3 -->
		<xiaochaoIndexedListData v-model:zuobiao='zuobiaoccc' :zishuju="shuju" @itemClick="onClick"></xiaochaoIndexedListData>
		<!-- #endif -->
		<!-- #ifdef VUE2 -->
		<xiaochaoIndexedListData :zuobiao.sync='zuobiao' :zishuju="shuju" @itemClick="onClick"></xiaochaoIndexedListData>
		<!-- #endif -->
	</view>
</template>
...
<script>
	...
	import objc from "@/uni_modules/xiaochao-indexed-list/components/xiaochao-indexed-list/shujuliebiao.js";
	...
	export default {
		...
		data() {
			return {
				...
				shuju: this.quanbushuju, //上个页面传过来的数据
				zimugaodu: 0,
				fenduan: 0,
				zuobiao: {},
				zuobiaoccc: 0,
		...
		created() {
			...
			// #ifdef APP-PLUS && VUE3
			this.zimugaodu = res.windowHeight - 44 - (uni.getSystemInfoSync().statusBarHeight); //设置右侧所有字母所占总高度
			// #endif
		...
		methods: {
			...
			touchstarta(e) {
				...
				this.isxianshi = true;
				let nagezimu = this.zimu[Math.floor((e.changedTouches[0].clientY) / this.fenduan)];
			...
			touchenda(e) {
				let _this = this;
				setTimeout(function() { //延迟下，否则快速点击不会出现中间黑色弹窗字母提示
